...
def test_sns_alert(_=Depends(require_admin)):
    """
    Admin-only: Send a test SMS via Amazon SNS to SNS_TEST_PHONE in .env.
    Use this to verify SNS integration is working before setting a live deadline.
    """
    test_phone = settings.SNS_TEST_PHONE
    if not test_phone or test_phone == "+919000000000":
        raise HTTPException(
            status_code=400,
            detail="Set SNS_TEST_PHONE in .env to a real E.164 number (e.g. +919876543210) first.",
        )

    from datetime import datetime
    msg = (
        f"JanSahayak AI Test Alert — SNS integration OK! "
        f"Sent at {datetime.now().strftime('%d %b %Y %H:%M')} IST. "
        f"Helpline: 1800-11-4000"
    )
    msg_id = sns_send_sms(test_phone, msg)
    if not msg_id:
        raise HTTPException(
            status_code=502,
            detail="SNS call failed — check AWS credentials, SNS permissions, and CloudWatch logs.",
        )
    return {
        "status": "ok",
